...
| AI 預測模型 | 台灣本土訓練資料 | 無海外競爭者有此資料集 |
| 離線同步協議 | 申請技術專利（三 Pass 對齊） | 做爛容易，做對難，你已踩過坑 |
...
| Wave | 內容 | 商業意義 | 狀態 |
|------|------|---------|------|
| **Wave 6** COP 完整實作 | 熱圖、叢集化、過濾搜尋、時間軸回放 | 提升指揮部態勢感知，加強與競品差距 | 🔲 待做 |
...
| 法規/標準 | 來源 | 對本系統的強制要求 |
|-----------|------|-------------------|
| 資通安全管理法 | 數位部 | 核心系統 Log ≥ 6 個月；年度弱掃 |
| 資通安全責任等級分級辦法 | 數位部 | 消防局屬 B 級，核心系統需 ISO 27001 |
| 個人資料保護法第 27 條 | 法務部 | 傷亡者個資需適當安全措施 |
| 政府資訊服務採購作業指引 | 數位部 + 工程會 | SBOM、陸資禁令、資安獨立預算 |
| 共通性資安基本要求（軟體） | 數位部 | MFA AAL2、SAST、滲透測試 |
| WCAG 2.0 AA | 數位部 | 政府系統新建/改版強制 |
| NIST SP 800-53 Rev. 5 | NIST（參考） | 稽核/存取控制/事件回應最佳實務 |
| ISO 22301:2019 | ISO（參考） | BCP、RTO/MTPD 定義 |
| FEMA CPG 101 v3.1 | FEMA（參考） | EOC 計畫書結構、多機關協同 |
| UN HXL / HDX | OCHA（參考） | NGO 客戶資料互通格式 |

---

*本文件為初步規劃草稿（v1.1），未設定時間點。各 Phase 的詳細工作拆解、工時估算，待確認執行優先序後另行制定。*
