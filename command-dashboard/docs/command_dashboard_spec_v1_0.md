...
**文件版本**：v1.5
**對應程式版本**：command-v0.6.10（commander_dashboard.html）
**狀態**：實作中
**最後更新**：2026-04-06
...
┌───────────────────────────────────────────────────────────┐
│ ZONE A — 態勢速讀（高度 64px）                             │
│ ┌────┐┌────┐┌──┐  12:34:57   收容 23 人  🔘收容 🔘醫療 🟢指揮│
│ │badge││注意││13│  04/06(日)  醫療 14 人       ⚙ v0.6.10  │
│ └────┘└────┘└DCI┘                                        │
├──────────┬──────────────────────────────┬─────────────────┤
│ 左側欄    │ ZONE B 中 — 互動地圖（~60%） │ 右側欄           │
│ (~20%)   │                              │ (~20%)          │
│          │  [演訓場地圖]                 │                 │
│ ① 量能   │                              │ ④ 物資          │
...
- 左側欄（態勢面，20%）：量能壓力、流向橋接、事件態勢
- 右側欄（事件面，20%，展開時 40%）：事件追蹤、待裁示
...
| 區塊 | 內容 |
|---|---|
| 左 | Badge（52px）+ 態勢燈（36px，RAG 三色，hover 顯示原因）+ DCI（hover 顯示各組資料信心） |
| 中（置中）| 時鐘 HH:MM:SS + 日期 MM/DD (星期) ｜ 收容 N 人 / 醫療 N 人（上下排列） |
| 右 | 連線燈號（收容/醫療/指揮部，統一 7px 圓點 + 文字）+ ⚙ 設定 + 版號 |

**態勢燈 hover tooltip**：顯示量能、事件、資源三維度的個別狀態。
**DCI hover tooltip**：顯示各組資料新鮮度（即時/稍舊/過時/未連線）+ 事件記錄數。
**連線燈號**：灰色（`lkp`）時旁邊顯示「未連線」，綠色時只顯示組別名稱。
**無快照時**：態勢燈根據事件嚴重度驅動，中區顯示未結事件數或「等待資料中」。
...
**Tab 樣式**：位於地圖中上方（`position:absolute; top:8px; left:50%`），選中為亮橘色（`rgba(240,136,62,.85)`），未選為半透明黑底。字體 10px。
...
事件放置流程（edit-bar，半透明 `rgba(22,27,34,.6)`）：
1. 選組別按鈕（前進/安全/收容/醫療/指揮部）
2. 選事件類型下拉
3. 點地圖放置 → 建立事件 + marker

拖曳 marker 調整位置 → 自動追加「地圖位置已移動」notes + `poll()`。
**刪除按鈕已移除**，事件只能透過事件欄結案流程結束（結案後 marker 自動從 map_config 移除）。
...
## 附錄 A　事件 Marker 交互規格（v0.6.0）

### A.1　統一交互模式（無齒輪切換）

| 操作 | 事件 marker | 據點 marker |
|------|-----------|-----------|
| 單擊 | 開啟事件處置 modal | 選取（綠框 + 刪除鈕） |
| 長按 >200ms | 拖曳移動 + 記錄 | 拖曳移動 |
| 雙擊 | — | 據點詳情 modal |
| hover | 上方顯示「組別　事件名」 | — |
| 點空白地圖 | 取消選取 / 放置事件 | 取消選取 |

### A.2　事件放置流程

```
底欄置中：[前進][安全][收容][醫療][指揮部] [—]
  1. 點組別 → 下拉自動展開
  2. 選事件類型 → 游標十字
  3. 點地圖 → 放置 + 建 DB + 退出放置模式
```
...
| 狀態 | 顏色 | 條件 |
|------|------|------|
| 新建 | 黃 `var(--yellow)` | 剩餘 > 總時間 50% |
| 警告 | 橘 `#e67e22` | 剩餘 < 總時間 50% |
| 逾時 | 紅閃 `var(--red)` | 超過 deadline |
| 結案 | 不渲染（消失） | resolved/closed |

Marker 編號格式：`EV-0XX`（去除日期），完整編號在 modal 內。

### A.4　事件處置 Modal

- 標題：icon + 短編號（EV-NNN）+ 事件名
- 狀態：severity 藥丸（緊急/警告/一般）+ status 藥丸 + 倒數
- 資訊：組別中文、回報人、發生時間、期限
- 處置紀錄：`HH:MM 操作者（金色 #DAA520）— 內容`，逾時倒數文字 pulse 閃爍
- 操作：輸入框（autocomplete=off）+ 三鈕等寬
  - 按鈕順序（符合 UI/UX 規格）：**關閉(ghost)** → 處理中(黃) → 結案(綠)
  - **Auto-Save**：點任何按鈕時自動儲存輸入框文字為 notes，不需要「追加」按鈕
  - 關閉 = 只儲存文字，不改狀態
- **結案限制**：無手動處置紀錄（排除系統狀態變更紀錄）→ 結案按鈕不可用
- **結案確認**：appConfirm 對話框確認後才執行
- **結案連動**：結案後自動從 map_config 移除對應 marker
- severity/status/組別/decision_type 一律中文

### A.5　事件追蹤欄

**標題列**：
- 左：◀/▶ 展開/收合箭頭 + 「事件追蹤」文字（點擊 toggle 組別分類）+ 未結案總數（黃色）+ 逾時數（紅色 pulse 閃爍）
- 右：統計列 — 待處理N（橘色，可點擊篩選）/ 處理N（黃色）/ 結案N（半透明），點擊 toggle 篩選
- 篩選啟用時顯示提示列 + ✕ 清除按鈕

**卡片設計**（PWA 風格）：
- 左邊色條 = **組別色**（前進 `#e68c1e` / 安全 `#3366e6` / 收容 `#d4b200` / 醫療 `#e03030` / 指揮部 `#22b05a`）
- 倒數文字色 = 動態色（黃→橘→紅，對齊 marker），逾時時 pulse 閃爍
- 組別名稱灰色 `var(--text3)`
- 事件編號短格式 `EV-NNN`（去除日期）
- 已結案：側邊條組別色半透明 + `55` alpha、opacity 0.55、標示「已結案」灰色
- **長按事件卡片**（600ms）→ 地圖自動切換並 highlight 該 marker（綠框 + 放大），其他 marker 灰掉（`.dimmed` opacity 0.15），放開恢復

**組別分類**：
- 點「事件追蹤」文字 → 按組別分組顯示，每組加標題 `── 前進組 ──`（組別色）
- 已結案區也支援組別分類
- 再點恢復時間序

**展開/收合**：
- 點 ◀/▶ 箭頭 → 左側收起、右側展 40%（地圖 60%）只顯示事件
- 點「⚖ 待裁示」標題列 → 左側收起、右側展 40% 只顯示裁示
- 再點 → 收回原狀（左 20% + 右 20%，6:4 分割）
- 展開狀態以 sessionStorage 保留過重刷
- 捲軸：JS 動態計算 max-height 確保滾輪可用

### A.6　重設期限 / 期限計算 / 後端防護

**重設期限**：三按鈕 +10分(紅) / +30分(黃) / +60分(綠)，從現在起算
...
**Event Code 唯一性**：
- `event_code` 欄位加 `UNIQUE INDEX`
- INSERT 用 subquery 原子產生流水號（`SELECT MAX + 1`），搭配 retry（最多 10 次）
- 併發測試驗證：5 人 × 200 筆，0 筆重複

**Notes 原子操作**：
- `add_event_note` 和 `update_event_status` 使用 SQLite `json_insert` 原子追加
- 避免併發 read-modify-write 覆蓋（舊版會丟失 notes）
- 併發測試驗證：5 人同時追加 15 筆 notes，0 筆丟失

**狀態機防護**：
- 合法轉換：`open → in_progress/resolved/closed`，`in_progress → resolved/closed`
- `resolved` / `closed` 不可逆
- 非法轉換回 HTTP 400

**Enum 驗證**：
- `severity`：`info` / `warning` / `critical`
- `reported_by_unit`：`shelter` / `medical` / `forward` / `security` / `command`
- 非法值回 HTTP 422

**狀態變更紀錄中文化**：
- 自動追加的 notes 使用中文（「狀態變更為『處理中』」而非 `in_progress`）

### A.8　狀態保留（sessionStorage）

| key | 用途 |
|-----|------|
| `_openEventId` | 重刷後恢復開啟中的事件 modal |
| `_currentMap` | 保留站內/站外地圖切換 |
| `_expandedSection` | 保留右側欄展開狀態 |

---

*文件版本：v1.5 | 狀態：實作中 | 對應程式：command-v0.6.10（commander_dashboard.html）*
