...
**文件版本**：v1.4
**對應程式版本**：command-v0.6.0（commander_dashboard.html）
**狀態**：實作中
**最後更新**：2026-04-05
...
- 資訊：組別中文、回報人、發生時間、期限
- 處置紀錄：`HH:MM 操作者 — 內容`
- 操作：輸入框+追加 / 處理中+結案+關閉（三鈕等寬）
- **結案限制**：無處置紀錄 → 結案 disabled
...
- 倒數文字色 = 動態色（黃→橘→紅，對齊 marker）
- 已結案側邊條 = 組別色 + `55` alpha（半透明）
- 組別名稱用組別色
- 已結案：側邊條組別色半透明、opacity 0.55、標示「已結案」
- 全站細捲軸 4px

**展開/收合**：
- 點「📋 事件追蹤」標題列 → 左側收起、右側展 50% 只顯示事件
- 點「⚖ 待裁示」標題列 → 左側收起、右側展 50% 只顯示裁示
- 再點同一列 → 收回原狀（左 18% + 右 18%，6:4 分割）
- 展開狀態以 sessionStorage 保留過重刷

### A.6　刪除 / 重設期限 / 期限計算

**刪除**：app 風格 confirm modal → 記錄 note + 結案事件
...
*文件版本：v1.4 | 狀態：實作中 | 對應程式：command-v0.6.0（commander_dashboard.html）*
