...
class TestSyncEmptyPayload:
    def test_empty_sync_push_returns_200(self, client):
        """空 payload（無 snapshots/events）→ 200，不崩潰"""
        r = client.post(_SYNC_URL, json=_make_sync_payload("shelter"))
        assert r.status_code == 200

    def test_empty_sync_does_not_change_db(self, client, tmp_db):
        """空 payload 不增加任何 snapshots"""
        from core.database import get_conn
        client.post(_SYNC_URL, json=_make_sync_payload("shelter"))
...
class TestSyncIdempotency:
    def test_same_payload_twice_idempotent(self, client, tmp_db):
        ...
        client.post(_SYNC_URL, json=payload)
        client.post(_SYNC_URL, json=payload)
        with get_conn() as conn:
            cnt = conn.execute("SELECT COUNT(*) as c FROM snapshots").fetchone()["c"]
        assert cnt == 2  # 不是 4

    def test_sync_pass1_updates_existing_snapshot(self, client, tmp_db):
        """
        ...
        r = client.post(_SYNC_URL, json=payload)
        assert r.status_code == 200
    ...
    def test_sync_result_consistent_on_retry(self, client, tmp_db):
        """sync push 失敗後重試（模擬網路重送）→ 結果與第一次相同"""
        from core.database import get_conn
        payload = _make_sync_payload("shelter", snapshots=[_base_snap("retry-001")])
        r1 = client.post(_SYNC_URL, json=payload)
        r2 = client.post(_SYNC_URL, json=payload)
...
class TestSyncOutOfOrder:
    def test_old_timestamp_via_sync_does_not_displace_latest(self, client, tmp_db):
        """
        ...
        # 先推新的
        client.post(_SYNC_URL, json=_make_sync_payload("shelter", snapshots=[
    ...
    def test_mixed_timestamps_all_stored(self, client, tmp_db):
        ...
        client.post(_SYNC_URL, json=_make_sync_payload("shelter", snapshots=snaps))
        with get_conn() as conn:
            cnt = conn.execute("SELECT COUNT(*) as c FROM snapshots").fetchone()["c"]
        assert cnt == 3
...
class TestSyncUnknownUnit:
    def test_unknown_source_unit_accepted(self, client):
        """未知 source_unit → 仍 200（sync 容錯設計，_unit_to_node fallback）"""
        r = client.post(_SYNC_URL, json=_make_sync_payload("unknown_unit", snapshots=[
            _base_snap("unknown-001")
        ]))
        assert r.status_code == 200

    def test_sync_without_snapshots_key(self, client):
        """payload 缺少 snapshots 欄位（只有 source_unit）→ 200 或 422，不崩潰"""
        r = client.post(_SYNC_URL, json={
            "source_unit": "shelter",
            "sync_start_ts": "2026-04-24T06:00:00Z",
            "device_id": "test",
        })
