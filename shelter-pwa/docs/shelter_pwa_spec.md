...
- 兩級權限（組長/一般），所有功能對一般使用者開放
- 家庭入站：一次建立多位成員，一鍵全家清消、套用同意/公約、全家離站
...
### 3.1　角色與權限（v2.2.21 更新）

> **【v2.2.1 設計變更】** 原四角色（組長/人管/物管/環管）簡化為兩級。人管/物管/環管之間無功能差異，操作追蹤由 `AuditManager.stamp()` 以 username 記錄，角色對可追溯性無貢獻。

| 層級 | 主要功能 | 高權限操作 |
|------|----------|-----------|
| 組長 ★YubiKey | 所有功能 + 設定頁 + 帳號管理 | 全站快照匯出、場復銷毀、帳號建立/停用、QR 通報產生 |
| 一般 | 儀表板、入站流程、床位地圖、物資管理、事件管理、名冊 | — |

- Server 啟動時自動將舊角色（人管/物管/環管）遷移為「一般」
- 降級模式使用 localStorage 快取角色，無快取則禁止登入

### 3.2　入站流程（v2.2.21 更新，6 步）

> **【v2.2 設計變更】** 原 11 步簡化為 6 步。合併同意+公約、即時需求+CMIST、標籤+識別證，移除造冊（自動產生）和帶位（床位分配即完成）。

**個人入站**：建立時填姓名/性別/年齡 → 進入步驟流程
**家庭入站**：建立時填所有成員姓名/性別/年齡 → 共同進入步驟流程

| 步驟 | Key | 名稱 | 說明 |
|------|-----|------|------|
| 1 | waiting | 等候 | 到達等候區 |
| 2 | san_gate | 清消 | 確認清消關卡 → 自動產生識別碼（S001-MA 格式） |
| 3 | consent_covenant | 同意簽署 | B軌（日常運作）+ A軌（撫恤）+ 收容公約。B軌拒絕→離站 |
| 4 | tagging_badge | 登記 | 管制物品寄存 + 識別證發放 + 寵物資訊 |
| 5 | needs_cmist | 需求評估 | 即時需求（6-8小時內）+ CMIST 特殊需求評估 |
| 6 | bed_assign | 床位分配 | 選擇床位 → 入站完成（已安置） |

**識別碼格式**：`S001-MA`（S=Shelter, 001=序號, M=性別代碼, A=年齡代碼）
- 性別代碼：M(男) / F(女)
- 年齡代碼：I(0-4) / J(5-19) / Y(20-34) / A(35-49) / M(50-64) / E(65+)

**人員狀態流轉**：等候中 → 登記中 → 已安置 → 已離站 / 已隔離

**操作員追蹤**：每完成一站記錄 `{step, by, at}` 至 `intake_log` 陣列

**離站處理**：
- 個人離站 / 全家離站
- 離站時自動結案所有關聯的未結事件
- 拒簽離站（同意簽署步驟）：B軌拒絕 → 以識別碼記錄後離站

### 3.3　QR-SHELTER 斷網快照通報（§9，v2.1 完整實作）
...
> **【指揮部儀表板依賴】** `srt_red`、`srt_yellow`、`srt_green` 是指揮部儀表板
> 「③ 橋接訊號圖」的資料來源。快照推送時必須包含這三個欄位，
> 指揮部才能監測收容端的脆弱性壓力何時轉移至醫療端。

| **pending_intake** | db.persons.count(status = "等候中") | — |
| **cmist_pending** | CMIST 自動分派事件未結案數 | — |
| **staff_on_duty** | `calcShelterActiveStaff()`：最近 30 分鐘不重複操作員數 | 自動推算，不再需要手動輸入 |
| **extra.staff_ratio** | 在場人數 / staff_on_duty | 人均服務量 |
| **extra.incident_pressure** | 最近 30 分鐘的事件統計 | high/medium/low/open_total/resolved_30min |
...
| 靜態加密（指揮部 Pi 500） | LUKS 全磁碟 + YubiKey | Phase 3 |
...
- 前提：指揮部 Pi 500 有穩定公網 IP 或 DDNS（DuckDNS / No-IP）
...
| 項目 | 狀態 | 說明 |
|------|------|------|
| Pi server：`site_salt` 管理 | ✅ 已完成 | `auth_result` 附帶 `site_salt`，Pi SQLite 持久化 |
| PWA：金鑰衍生改用 site_salt | ✅ 已完成 | 正常登入路徑已使用 site_salt；IndexedDB 快取供 QR 應急模式使用 |
| PWA：降級登入路徑移除 | 🔲 待實作 | `shelter_enc_salt_degraded` 裝置本地 salt 路徑仍存在，需移除 |
...
| 項目 | 狀態 | 說明 |
|------|------|------|
| 藥品管制介面（完整記錄欄位 + 提醒機制） | ⏳ 待協議 | 待收容組/醫療組協議流程 |
...
> **【v2.2新增】** 整合自 security_network_spec_v1_0.md §2.2；v2.3 版號更新至 v1.2。已改用 Pi 500 為主要硬體；更新 Lexar、D-Link、Anker 評等。

### A.1　建議使用（非中國品牌）

| 產品 | 品牌/原產地 | 用途 |
|------|-------------|------|
| Raspberry Pi 500 | Raspberry Pi Foundation，英國 | 指揮部 Console、各組 Pi（WebSocket server） |
...
| **Beelink Mini PC** | 深圳品牌（SZBox），已改用 Pi 500 為主要硬體；N100 角色待定 |
...
| 4 | **實作路線圖 Phase 1/2/3** | 明確區分 Mac 開發 / Pi 部署 / 指揮部 Pi 500 全整合三個階段 | §8.2–8.4 |
| 5 | **硬體安全清單更新** | 改用 Pi 500 為主要硬體（指揮部、收容組、醫療組）；N100 保留待定；Lexar 列禁用；D-Link、Anker 列個案評估 | 附錄 A |
| 6 | **資料所有權說明** | 明確各層資料所在，LUKS 為強制要求 | §7.2 |
| 7 | **待確認事項更新** | 補充 N100 角色待定、Anker、指揮官版儀表板等待決定項目 | §8.5 |
...
### v2.5 變更（對應程式 v2.2.21 / server v0.3.0）

| # | 變更項目 | 說明 | 影響範圍 |
|---|----------|------|----------|
| 1 | **角色簡化為兩級** | 原四角色（組長/人管/物管/環管）改為組長/一般；server 啟動自動遷移舊角色 | §3.1 |
| 2 | **入站流程簡化至 6 步** | 原 11 步合併簡化：同意+公約、即時需求+CMIST、標籤+識別證；移除造冊和帶位 | §3.2（新增） |
| 3 | **識別碼最少特徵編碼** | 清消後自動產生 S001-MA 格式（S=Shelter, 序號, 性別代碼, 年齡代碼） | §3.2 |
| 4 | **人員狀態簡化** | 移除「待分配」「待帶位」，流程：等候→登記→已安置→已離站/已隔離 | §3.2 |
| 5 | **操作員追蹤** | 每步驟記錄 intake_log（step/by/at）；audit 欄位改用 username 取代角色 | §3.2 |
| 6 | **離站自動結案事件** | 離站時自動結案所有關聯的未結事件 | §3.2 |
| 7 | **全家離站** | 家庭成員離站 modal 新增「全家離站」按鈕 | §3.2 |
| 8 | **名冊家庭分組** | 名冊頁家庭成員框在一起顯示，已離站成員半透明 | §3.2 |
| 9 | **事件 assigned_to** | CMIST 自動派遣改為指派給當前操作員（非角色名） | 附錄 B |
| 10 | **B軌拒簽擋住** | 同意簽署步驟 B 軌拒絕時無法繼續，必須走「拒簽離站」 | §3.2 |

### v2.6 變更（對應程式 v2.2.39-alpha / server v0.3.0）

| # | 變更項目 | 說明 | 影響範圍 |
|---|----------|------|----------|
| 1 | **事件結案連動確認** | 結案時彈出確認，詢問是否恢復涉及人員健康標記（不限事件類型） | §3.2 |
| 2 | **事件嚴重度預設** | air_raid/security_threat/medical_infection/medical_transfer 預設緊急 | 附錄 B |
| 3 | **新增事件涉及人員下拉選單** | 替代手動輸入 display_id，列出所有非離站人員 | 事件管理 |
| 4 | **移除 covenant_refused 事件類型** | 拒簽走「拒簽離站」流程，不建事件 | 附錄 B |
| 5 | **人員↔事件雙向連結** | 人員詳情顯示關聯事件（可點擊），事件涉及人員可點擊跳轉 | §3.2 |
| 6 | **健康標記需關注事件類型可選** | 不再自動建 resident_distress，彈出 modal 讓操作員選類型 | §3.2 |
| 7 | **紀錄格式統一** | 所有時間+操作員紀錄統一為「時間 操作員 — 內容」格式 | 全域 |
| 8 | **UX 一致性** | 灰階統一 0.5、按鈕統一 flex-1、卡片圓角統一 var(--r)、顏色改用 CSS 變數 | 全域 |
| 9 | **展示模式** | GitHub Pages 免 Pi 試用，自動偵測顯示展示模式按鈕 | 新增 |
| 10 | **UI/UX 設計規範文件** | 新增 ui_ux_design_guide.md | docs |
...
| # | 項目 | 優先級 |
|---|------|--------|
| 1 | Modal 返回層級錯誤（需實作 modal stack） | 高 |
| 2 | 家庭入站性別/年齡按鈕太小 | 中 |
| 3 | 表單缺即時驗證 | 中 |
| 4 | Tab 切換丟失捲動位置 | 中 |
| 5 | 全域人員搜尋（跨 tab） | 中 |
