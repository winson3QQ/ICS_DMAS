---
name: medical PWA 待處理問題清單（2026-04-01）
description: medical_pwa.html v0.2.0 的已知問題、優先順序、修改方向，演習前必修 P1/P2/P7/P8
type: project
---
...
### P2 — `nextDisplayId()` 產生重複 ID
- 問題：`_state.patients.length + 1` 只算活躍傷患，已後送者不計入，導致序號重複
- 修正：改為 async，從 DB `orderBy('id').last()` 取最大值
- ID 格式改為 `M007-MA`（含性別年齡代碼）
- 需同步升級 DB 到 version 4，加入 `source_sex`、`source_age`、`arrival_mode` 欄位
...
- 修正：DB 加 `arrival_vitals` 欄位（納入 P2 的 v4 migration），patient card 分兩行顯示

### Badge 定義（從 memory 補入，issues.md 未收錄）
- 接收 tab badge → 建議移除（檢傷官自己建單不需提醒）
- 分區 tab badge → 建議改為「待評估 + 逾時合計」
- 分區看板「全部待評估」篩選 tab → 待確認是否需要
- **尚未決定**

### 治療評估 modal 重繪驗證（從 memory 補入，issues.md 未收錄）
- `renderTreatmentModal` 用 `document.getElementById('modal-body')` 判斷是否已開啟
- 切換傷型時是否正常重繪，需實際測試驗證
...
- 離線備援：regex 規則解析；有網路時用 Claude API 解析
- Pi 5 需額外開 WebSocket endpoint 接收音訊（與資料同步 endpoint 分開）
- **P1–P8 完成後再做**
...
| 🔴 | P8 全域掃碼查詢 | 演習前 |
| 🟡 | P3 紅區容量（待決定方案） | 演習前確認 |
| 🟡 | P4 到院生命徵象分開顯示 | 演習前（時間允許） |
| 🟡 | Badge 定義 | 演習前確認 |
| 🟡 | 治療評估 modal 重繪 | 演習前驗證 |
| 🟢 | P5/P6/P9 | 演習後 |

**Why:** issues.md 整理於 2026-04-01，涵蓋 v0.2.0 的所有已知問題，是演習前開發的主要依據。
