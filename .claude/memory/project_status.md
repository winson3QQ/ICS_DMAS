...
---
## 上次做到哪裡（2026-04-25，C1-A Phase 2 Steps 1–5 完成，cmd-v2.1.0 / frontend v0.13.0）

### C1-A Phase 2 Steps 1–5 完成項目

| Step | 內容 | 狀態 |
|------|------|------|
| Step 1 | M006 Migration：`accounts.deleted_at` soft delete + `operational_periods` + `duty_log` 表 | ✅ |
| Step 2 | `account_repo.py`：`VALID_ROLES`、`get_account()`、soft delete、`ensure_initial_admin_token` 改 系統管理員、`is_first_run_required` 修正 | ✅ |
| Step 3 | `auth/rbac.py`：`ROLE_HIERARCHY` + `require_role()` FastAPI Dependency | ✅ |
| Step 4 | `routers/admin.py`：`_require_admin`（系統管理員 session 優先 + Admin PIN break-glass）+ `_require_commander_or_pin` | ✅ |
| Step 5 | 全端點 role gate：events/decisions/exercises/ttx/ai/config/map/audit_log；前端設定頁角色可見性；`_check_no_escalation` 防越權；帳號管理 UI 移除刪除按鈕、限制 toggle 編輯；觀察員 read-only 模態框；30 security tests | ✅ |

**版本**：`APP_VERSION=2.1.0`、`CMD_VERSION=v0.13.0`
**測試**：`tests/security/test_admin_gate.py` 30 passed

### C1-A Phase 2 Steps 6–12 待做

| Step | 內容 |
|------|------|
| Step 6 | Web 還原 v1（flag file + lifespan）|
| Step 7 | UI 小修（UTC→本地時間、423 toast 區分）|
| Step 8 | Backup UI 整合進 commander_dashboard.html，刪 admin_backups.html |
| Step 9 | 兩組 PIN 命名修正 |
| Step 10 | operational_periods + duty_log + Transfer of Command API |
| Step 11 | ICS role_detail 下拉選單 |
| Step 12 | 所有測試補齊 |
