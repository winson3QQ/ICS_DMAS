...
- 管制物資優先列示：止血類、呼吸道類、循環類、藥品、固定類（軟式護木、三角巾、頸圈）、保暖類（保暖棉被、鋁箔毯）、AED / 監測器材、防護用品、紀錄類（救護紀錄表、檢傷卡、筆）。
...
| v0.3 | 前端骨架 | PWA、IndexedDB、角色頁、接收/檢傷、治療區看板 | 規格先行（實作尚未開始） |
| v0.4 ★ | SOP 對齊補強 | §2.3 補入 XABCDE（創傷）與 SAMPLE/OPQRST（非創）欄位；§3.1 補入機動人員角色；§6.2 補入 2 小時輪替提醒與 4 小時強制交班警示 | 規格先行 |
| v0.5 | 作戰核心 | 再評估倒數、後送頁、物資頁、事件頁、交班快照 | 待定 |
| v0.5 | 同步與資安 | PIN 鎖、匯出加密、Peer Delta / WebSocket、mkcert 部署驗證 | 待定 |
| v1.0 | 演訓可用版 | 離線穩定、列印交班摘要、資料凍結、缺陷修正 | 待定 |
...
*文件版本：v0.4 | 對應程式版本：尚未開始實作 | 整合來源：醫療組SOP_20260323.md、shelter_pwa_spec_v2_3.md、security_network_spec_v1.2.md*
